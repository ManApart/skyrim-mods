Scriptname AKPSFurnitureHeldScriptHF extends ObjectReference  
...
if inventory == false
	...
	if  button == 0 
		...
		index = -1
		imax =  akpsfurnitureheldhf.getsize() - 1

		While (self.getbaseobject() as book) != akpsfurnitureheldhf.getat(index) && index <= imax
			index = index + 1
		endwhile 
		if (index > imax)
		...
		akfurnitureitem = akpsfurniturehf.getat(index) as furniture
