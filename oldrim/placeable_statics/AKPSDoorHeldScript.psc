Scriptname AKPSDoorHeldScript extends ObjectReference  
...
if inventory == false
	...
	if  button == 0 
		...
		index = -1
		imax =  akpsdoorheld.getsize() - 1

		While (self.getbaseobject() as book) != akpsdoorheld.getat(index) && index <= imax
			index = index + 1
		endwhile
		if (index > imax)
		...
		akfurnitureitem = akpsdoor.getat(index) as door
