Scriptname AKPSStaticHeldScript extends ObjectReference  
...
if inventory == false
	...
	if  button == 0 
		...
		index = -1
		imax =  akpsfurniturestaticheld.getsize() - 1

		While (self.getbaseobject() as book) != akpsfurniturestaticheld.getat(index) && index <= imax
			index = index + 1
		endwhile
		if (index > imax)
		...
		akfurnitureitem = akpsfurniturestatic.getat(index) as activator
